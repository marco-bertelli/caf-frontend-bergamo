...
import { USER_TYPES } from 'src/store/types'
...
export default {
  ...
  setup () {
    const entity = 'users'
    const types = {
      filters: USER_TYPES.FILTERS,
      pagination: USER_TYPES.PAGINATION
    }
  ...
  methods: {
    goToDetail (row) {
      this.$router.push({ name: 'usersDetail', params: { id: row?._id } })
    },
    onClickCreate () {
      this.$router.push({ name: 'usersDetail', params: { id: 'new' } })
    },
    onClickDelete (row) {
      this.$q.dialog({
        ...
        componentProps: {
          ...
          onConfirm: async () => {
            await this.$axios.delete(`users/${row?._id}`)
          },
          onSuccess: async () => {
